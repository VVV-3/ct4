...
#                 'F3-C3', 'C3-P3', 'P3-O1', 'Fp2-F4', 'F4-C4', 'C4-P4', 'P4-O2', 'Fz-Cz', 'Cz-Pz']
    group = 'EEG'
    num_channels = 9
    bsr_features = float('nan') * np.ones(num_channels)
    bsr_features2 = float('nan') * np.ones(num_channels)
...
#             print(len(channels))
            if all(channel in channels for channel in eeg_channels):
                ...
                data = np.array([data[7, :] - data[5, :], 
                                 ...
                                 data[16, :] - data[18, :], 
                                 data[18, :] - data[11, :],
                                 data[7, :]  - data[3, :]]) # Convert to bipolar montage: F3-P3 and F4-P4
...
#                 print(bsr_features.shape)
                ...
                eeg_features = float('nan') * np.ones(num_channels*4) # 2 bipolar channels * 4 features / channel
        else:
            eeg_features = float('nan') * np.ones(num_channels*4) # 2 bipolar channels * 4 features / channel
    else:
        eeg_features = float('nan') * np.ones(num_channels*4) # 2 bipolar channels * 4 features / channel
...
#             print(len(channels))
            if all(channel in channels for channel in eeg_channels):
                ...
                data = np.array([data[7, :] - data[5, :], 
                                 ...
                                 data[18, :] - data[11, :],
                                 data[7, :]  - data[3, :]]) # Convert to bipolar montage: F3-P3 and F4-P4
                bsr_features2 = bsr(data, sampling_frequency)
#                 print(bsr_features.shape)
                eeg_features2 = get_eeg_features(data, sampling_frequency).flatten()
            else:
                eeg_features2 = float('nan') * np.ones(num_channels*4) # 2 bipolar channels * 4 features / channel
        else:
            eeg_features2 = float('nan') * np.ones(num_channels*4) # 2 bipolar channels * 4 features / channel
    else:
        eeg_features2 = float('nan') * np.ones(num_channels*4) # 2 bipolar channels * 4 features / channel
    ...
    # Extract features.
    return np.hstack((patient_features, eeg_features, eeg_features2, bsr_features, bsr_features2, ecg_features))
...
#     print(filtered_data.shape)
    ...
    epochs = filtered_data[:, :n_epochs * epoch_samples].reshape(len(filtered_data), n_epochs, epoch_samples)
...
#     print(features.shape, num_channels)
